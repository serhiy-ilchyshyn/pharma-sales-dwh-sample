...
| Семантична модель | TMDL готовий; деплой через git integration (айтем, створений повз git, зникає при синхронізації) |
...
| 2 | Задеплоїти семантичну модель через git | закомітити `fabric-semantic-model/` у `main` → Source control → Update all. REST API у цьому воркспейсі не підходить: sync прибирає айтем, а create падає на привʼязці джерела |
