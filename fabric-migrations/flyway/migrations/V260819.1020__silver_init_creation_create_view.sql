...
WITH raw_region AS (
    SELECT region FROM [lhbronze].[erp_erp].[CUSTOMERS]
    UNION ALL
    SELECT region FROM [lhbronze].[erp_erp].[DOCTORS]
    UNION ALL
    SELECT region FROM [lhbronze].[erp_erp].[WAREHOUSES]
    UNION ALL
    SELECT region FROM [lhbronze].[erp_erp].[ADVERSE_EVENTS]
)
SELECT DISTINCT
...
WITH raw_city AS (
    SELECT region, city FROM [lhbronze].[erp_erp].[CUSTOMERS]
    UNION ALL
    SELECT region, city FROM [lhbronze].[erp_erp].[DOCTORS]
    UNION ALL
    SELECT region, city FROM [lhbronze].[erp_erp].[WAREHOUSES]
),
dedup_city AS (
    SELECT DISTINCT
...
FROM [lhbronze].[erp_erp].[EMPLOYEES] AS e
...
FROM [lhbronze].[erp_erp].[CUSTOMERS] AS c
...
FROM [lhbronze].[erp_erp].[PRODUCTS] AS p
...
FROM [lhbronze].[erp_erp].[DOCTORS] AS d
...
WITH latest_warehouse AS (
    ...
    FROM [lhbronze].[erp_erp].[WAREHOUSES] AS w
)
SELECT
